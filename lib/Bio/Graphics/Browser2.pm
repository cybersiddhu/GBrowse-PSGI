...
sub resolve_path {
    ...
    $self->can($method)
        or confess "path_type must be one of 'config','htdocs', or 'url'";
...
sub url_path {
    my $self   = shift;
    my $option = shift;
    $self->resolve_path( scalar( $self->setting( general => $option ) ),
        'url' );
}
...
sub tmp_base {
    eval { shift->setting( general => 'tmp_base' ) }
...
sub persistent_base {
    ...
    return $base
        || $self->tmp_base;    # for compatibility with pre 2.27 installs
}
...
sub db_base {
    ...
    return $base
        || File::Spec->catfile( shift->persistent_base, 'databases' );
}
...
sub slave_dir {
    my $self = shift;
    my $path = $self->setting( general => 'tmp_slave' )
        || '/tmp/gbrowse_slave';
    $self->make_path($path) unless -d $path;
    return $path;
}
...
sub application_name {
    shift->setting( general => 'application_name' ) || 'GBrowse';
}

sub application_name_long {
    shift->setting( general => 'application_name_long' )
        || 'The Generic Genome Browser';
}

sub email_address {
    shift->setting( general => 'email_address' ) || 'noreply@gbrowse.com';
}

sub smtp {
    my $smtp = shift->setting( general => 'smtp_gateway' );
    return if $smtp eq 'none';
    return $smtp;
}
sub smtp_enabled { return defined shift->smtp; }

sub user_account_db {
    shift->setting( general => 'user_account_db' );
}    # Used by uploads & user databases, they set their own defaults.
...
sub upload_db_adaptor {
    my $self = shift;
    my $adaptor = $self->setting( general => 'upload_db_adaptor' )
        || $self->setting( general => 'userdb_adaptor' );
    warn
        "The upload_db_adaptor in your Gbrowse.conf file isn't in the DBI::<module> format: remember, it's not a connection string."
...
sub upload_db_host {
    my $self = shift;
    return
           $self->setting( general => 'upload_db_host' )
        || $self->setting( general => 'userdb_host' )
        || 'localhost';
}
...
sub upload_db_user {
    my $self = shift;
    return
           $self->setting( general => 'upload_db_user' )
        || $self->setting( general => 'userdb_user' )
        || '';
}
...
sub upload_db_pass {
    my $self = shift;
    return
           $self->setting( general => 'upload_db_pass' )
        || $self->setting( general => 'userdb_pass' )
        || '';
}
...
sub session_args {
    my $self = shift;
    my %args
        = shellwords( $self->setting( general => 'session args' ) || '' );
    return \%args if %args;
...
sub data_source_show {
    ...
    local $self->{'.authenticator'} = $authenticator
...
sub data_source_path {
    ...
    my ($regex_key)
        = grep { $dsn =~ /^$_$/ }
        map { $_ =~ s/^=~//; $_ }
        grep { $_ =~ /^=~/ } keys( %{ $self->{config} } );
    if ($regex_key) {
        my $path = $self->resolve_path(
            $self->setting( "=~" . $regex_key => 'path' ), 'config' );
...
sub get_source_from_cgi {
    my ( $self, $req ) = @_;
	$req ||= $self->req;
	croak "Plack::Request is not set yet\n" if !$req;
...
sub authorized_session {
    ...
    if ( $session->match_nonce( $authority, $self->req->remote_host() ) ) {
        return $session;
    }
    else {
