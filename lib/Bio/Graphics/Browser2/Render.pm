...
my $FCGI_REQUEST;    # stash fastCGI req handle
...
sub new {
    ...
    my ( $data_source, $session, $reqed_id, $authority , $req);

    if ( @_ == 2 ) {
        ( $data_source, $session ) = @_;
    }
    elsif (@_ == 3) {
        ( $data_source, $session , $req) = @_;
    }
#    elsif ( @_ == 1 ) {
#        my $globals = shift;
#        $reqed_id = param('id')        || CGI::cookie('gbrowse_sess');
#        $authority    = param('authority') || CGI::cookie('authority');
#        my $shared_ok = Bio::Graphics::Browser2::Action->shared_lock_ok(
#            param('action') );
#        $session = $globals->authorized_session( $reqed_id, $authority,
#            $shared_ok );
#        $globals->update_data_source($session);
...
#    }
        ...
        croak "usage: "
            . __PACKAGE__
            . "->new(\$globals) or ->new(\$data_source,\$session)";
    }
...
# Bio::Graphics::Browser2::Render->req->{name}
sub request {
    return $STATE;
}
...
sub userdb {
    ...
    my $userdb = $self->{userdb}
        ||= Bio::Graphics::Browser2::UserDB->new( $self->globals );
    return $userdb;
}
...
sub get_usertrack_labels {
    ...
    my @files
        = $userdata->tracks;    # misnomer -- should be "uploads" or "files"
    for my $f (@files) {
...
sub debug {
    ...
    return $self->{debug} = DEBUG
        || $self->data_source->global_setting('debug');
}
...
sub run {
    my ( $self, $req ) = @_;
    $req ||= $self->req;
    croak "Need a Plack::Request object\n" if !$req;
...
# authentication required, but not a login session, so initiate authentication req
    ...
    if ( my $file = $req->param('share_link') ) {
    ...
    if ( $req->param('reset') ) {
        $self->response->redirect( $req->base );
    }
    else {
...
sub user_authorized_for_source {
    ...
    return $globals->authorized( $source->name, $username,
        $plugins->auth_plugin )
...
sub set_source {
    ...
    my $req    = $self->req;
    if ( $req->path_info ne "/$source/" ) {
        my $args = $self->to_query_string($req->parameters);
        my $url  = $req->path_info;
...
sub run_asynchronous_event {
    ...
    warn
        "[$$] asynchronous event returning status=$status, mime-type=$mime_type"
        if TRACE_RUN;
    ...
    if ( $status == 302 ) {    # redirect
        $resp->redirect( $self->req->base . $data );
    }
...
#
# Asynchronous reqs. See Bio::Graphics::Browser2::Action for dispatch
# table. Each req returns a three element list of format:
...
sub asynchronous_event {
    ...
    warn "[$$] asynchronous event(", $self->to_query_string($self->req->parameters), ")" if TRACE_RUN;

    # TO ADD AN ASYNCHRONOUS REQUEST...
    # 1. Give the req a unique name, such as "foo"
    ...
    #    ACTION_foo that processes the req. The method will receive
    ...
    if ( my @result = $dispatch->handle_legacy_calls( $req, $self ) ) {
        return @result;
    }

    if ( my $track_name = $req->param('display_citation') ) {
        my $html = $self->display_citation($track_name);
        return ( 200, 'text/html', $html );
    }

    elsif ( my $action = $req->param('action') ) {
...
sub authorize_user {
    ...
    FIX: {
            ...
            $userdb->set_session_and_uploadsid( $userid, $session->id,
                $uploadsid );
            $id = $session->id;
        }
    }
...
sub background_track_render {
    ...
    if ( $self->get_panel_renderer( $self->segment )->vis_length
        <= $self->get_max_segment )
    {
        $reqs{'detail'} = $self->render_deferred(
        ...
        $details_msg     = h1(
            ...
            $self->translate(
                'TOO_BIG',
                scalar $self->data_source()
                    ->unit_label( $self->get_max_segment ),
            )
        );
    }

    $reqs{'region'} = $self->render_deferred(
        labels  => [ $self->expand_track_names( $self->regionview_tracks ) ],
        ...
        && $self->data_source->show_section('region') );

    $reqs{'overview'} = $self->render_deferred(
        labels  => [ $self->expand_track_names( $self->overview_tracks ) ],
    ...
    for my $section ( keys %reqs ) {
        my $cache_track_hash = $reqs{$section};
        foreach my $track_label ( keys %{ $cache_track_hash || {} } ) {
            my $track_id = $self->trackname_to_id( $track_label, $section );
            $track_keys{$track_id} = $cache_track_hash->{$track_label}->key();
        }
    }

    warn "background_track_render() return track keys ", join ' ', %track_keys
        if DEBUG;
    ...
    if ($segment) {
        foreach my $track_name (@$track_names) {
            ...
            for my $track_id (@track_ids) {
                ...
                my $track_section
                    = $self->data_source->get_section_from_label($track_id);
                ...
                warn
                    "add_track() returning track_id=$track_id, key=$track_key, name=$track_name, panel_id=$panel_id"
                    if DEBUG;
...
sub create_cache_extra {
    ...
    my $settings    = $self->state();
    my @cache_extra = (
        $settings->{show_tooltips}, $settings->{ref},
        $settings->{start},         $settings->{stop},
    );
...
sub background_individual_track_render {
    ...
    if (   $section eq 'detail'
        && $self->segment
        && $self->get_panel_renderer( $self->segment )->vis_length
        > $self->get_max_segment() )
    {
        $display_details = 0;
        $details_msg     = h1(
            $self->translate(
                'TOO_BIG', scalar $source->unit_label(MAX_SEGMENT),
            )
        );
...
sub state_cookie {
    ...
    my $cookie  = {
        value   => $id,
        path    => $path,
        expires => $globals->time2sec( $globals->remember_settings_time ),
    };
    return $cookie;
}
...
sub auth_cookie {
    ...
    my $path = $self->req->path_info;
    $path =~ s!gbrowse/?$!!;
    my $auth     = ( shift || $self->req->param('authority') ) or return;
    my $globals  = $self->globals;
    ...
    if ($remember) {
        $cookie->{expires}
            = $globals->time2sec( $globals->remember_settings_time );
    }
    return $cookie;
}
...
sub create_cookie {
    my $self = shift;
    return [ $self->state_cookie, $self->auth_cookie ];
}
...
sub render_body {
    ...
    if ( $region->feature_count > 1 ) {
        $main_page .= $self->render_multiple_choices( $features,
            $self->state->{name} );
        $main_page .= $self->render_select_track_link;
    }

    elsif ( my $seg = $region->seg ) {
        $main_page .= $self->render_panels(
            $seg,
            {   overview   => $source->show_section('overview'),
    ...
    elsif ( $region->feature_count > 0 )
    {    # feature but no segment? Admin error
        my $message = $self->translate('CHROM_NOT_FOUND');
        my $details = $self->translate(
            'CHROM_NOT_FOUND_DETAILS',
            $features->[0]->display_name,
            $features->[0]->seq_id
        );
        $main_page .= script( { -type => 'text/javascript' },
            "Controller.show_error('$message','$details')" );
    }

    my $tracks    = $self->render_tracks_section;
    my $snapshots = $self->snapshot_manager->render_snapshots_section;
    my $community
        = $self->user_tracks->database
        ? $self->render_community_tracks_section
        : "";
    my $custom        = $self->render_custom_tracks_section;
    my $global_config = $self->render_global_config;

    $output .= $self->render_tabbed_pages( $main_page, $tracks, $snapshots,
        $community, $custom, $global_config );
...
sub generate_title {
    ...
    return
         !$features || !$state->{name} ? $description
...
sub segment_info_object {
    ...
    my $pad 
        = $self->data_source->global_setting('pad_left')
    ...
    my %segment_info_object = (
        ...
        'ref'                => $segment->seq_id,
        details_pixel_ratio  => $segment->length
            / ( $state->{width} * $self->details_mult() ),
        detail_width   => $detail_width,
        overview_width => $image_width,
        width_no_pad   => $state->{width},
        details_mult   => $self->details_mult(),
        hilite_fill    => $self->data_source->global_setting('hilite fill')
            || 'red',    # Not sure if there's a
        hilite_outline => $self->data_source->global_setting('hilite outline')
            || 'gray',    # better place for this
        flip               => $state->{flip},
        initial_view_start => $state->{view_start},
        initial_view_stop  => $state->{view_stop},
        length_label       => scalar $self->data_source->unit_label(
            $state->{view_stop} - $state->{view_start}
        ),
...
sub render_panels {
        ...
        $html .= div(
            $self->toggle(
                ...
                div({   -id    => 'overview_panels',
                        -class => 'track',
                        -style =>
                            'margin-bottom:3px; overflow: hidden; margin-left:auto; margin-right:auto; position:relative; width:'
                            . $self->get_image_width( $self->state ) . 'px'
                    },
                    $scale_bar_html,
                    $panels_html,
                )
    ...
    if ( $section->{'detailview'} ) {
        ...
        my $panels_html
            = $self->get_blank_panels( [ $self->detail_tracks ], 'detail' );
        ...
        $html .= div(
            $self->toggle(
                { tight => 1 },
                'Details',
                div({   -id    => 'detail_panels',
                        -class => 'track',
                        -style =>
                            'margin-left:auto; margin-right:auto; position:relative; width:'
                            . $self->get_image_width( $self->state ) . 'px'
                    },
                    $details_msg,
                    $ruler_html,
...
sub region {
    ...
    if ( $current_plugin && $plugin_action eq $self->translate('Find')
        || lc $plugin_action eq 'find' )
    {
        $region->features(
            $self->plugin_find( $current_plugin, $self->state->{name} ) );
    }
    elsif ( $self->state->{ref} ) {    # a known region
        $region->set_features_by_region(
            @{ $self->state }{ 'ref', 'start', 'stop' } );
    }
    elsif ( my $features = $self->plugin_auto_find( $self->state->{name} ) )
    {    # plugins with the auto_find() method defined
...
sub thin_segment {
    ...
    if ( defined $state->{ref} ) {
        return Bio::Graphics::Feature->new(
            -seq_id => $state->{ref},
            -start  => $state->{start},
            -end    => $state->{stop}
        );
...
sub thin_region_segment {
    ...
    return Bio::Graphics::Browser2::Region->region_segment( $thin_segment,
        $state, $thin_whole_segment );
}
...
sub region_segment {
    ...
    my $settings = $self->state;
    return $self->{region_segment}
        = $self->region->region_segment( $segment, $settings,
        $self->whole_segment );
}
...
sub init_plugins {
    ...
    return $PLUGINS{$source}
        if $PLUGINS{$source} && $self->{'.plugins_inited'}++;
...
sub plugin_auto_find {
    ...
    return
        if Bio::Graphics::Browser2::Region->is_chromosome_region(
        $search_string);
...
# Write informative information into the search box - not sure if this is the right thing to do.
    $settings->{name}
        = defined($search_string)
        ? $self->translate( 'Plugin_search_1', $search_string, $plugin_name )
        : $self->translate( 'Plugin_search_2', $plugin_name );
...
sub handle_gff_dump {
    ...
    # new API
    if ( my $action = $req->param('f') || $req->param('fetch') ) {
        $gff_action = $action;
    }
    elsif ( $action = $req->param('gbgff') || $req->param('download_track') )
    {
        ...
        $gff_action .= " trackdef"
            if $req->param('s')
                or $req->param('stylesheet');
    }
    return unless $gff_action;

    my %actions = map { $_ => 1 } split /\s+/, $gff_action;
    my $segment = $req->param('q') || $req->param('segment') || undef;

    my @labels = $self->split_labels_correctly( param('l') );
    @labels = $self->split_labels( ( param('type'), param('t') ) )
        unless @labels;
    @labels = $self->visible_tracks unless @labels;

    $self->state->{preferred_dump_format} = param('format')
        if param('format');
    ...
    # so that another user's tracks are added if reqed
    $self->add_user_tracks( $self->data_source, param('uuid') )
        if param('uuid');
    ...
    if ( $actions{scan} ) {
        ...
        if ( $actions{save} && ( $actions{datafile} || $actions{gff3} ) ) {
            ...
            $resp->content_type('text/plain');
            $resp->body( $dumper->print_stylesheet() );
        }
        else {
            $resp->content_type($mime);
            $resp->headers(
...
sub handle_track_dump {
    ...
    $req->param('show_tracks') or return;
    $resp->content_type('text/plain');

    my ( %ts, %ds, @labels_to_dump );
    if ( my @labels
        = $source->track_source_to_label( shellwords $req->param('ts') ) )
    {
        %ts = map { $_ => 1 } @labels;
    }
    if ( my @labels
        = $source->data_source_to_label( shellwords $req->param('ds') ) )
    {
        %ds = map { $_ => 1 } @labels;
    }
    if ( $req->param('ts') && $req->param('ds') ) {    # intersect
        @labels_to_dump = grep { $ts{$_} } keys %ds;
    }
    elsif ( $req->param('ts') or $req->param('ds') ) {    #union
    ...
    print '#',
        join( "\t", qw(TrackLabel DataSource TrackSource Description) ), "\n";
...
sub handle_plugins {
    ...
    warn
        "an operation was reqed on plugin $id, but this plugin has not been configured"
        unless $plugin;
    ...
    if ( param('plugin_do') ) {
        $plugin_action = $self->translate( param('plugin_do') )
            || $self->translate('Go');
    }
    ...
    {
        $segment->{start} = param('view_start')
            || $segment
            ->{start};   # We only care about the segment the user is actually
        $segment->{stop} = param('view_stop')
            || $segment
            ->{stop};  # viewing, not the whole segment that he/she has loaded
...
sub init_remote_sources {
    ...
    warn "init_remote_sources()" if DEBUG;
    my $remote_sources
        = Bio::Graphics::Browser2::RemoteSet->new( $self->data_source,
        $self->state, $self->language, $self->session->uploadsid, );
...
sub render_galaxy_form {
    my $self = shift;
    my $seg  = shift;
    $self->wrap_in_div( 'galaxy_form', $self->galaxy_form($seg) );
}
...
sub write_auto {
    ...
    for my $f (@$features) {
        ...
        $feature_file .= "\nreference=$reference\n";
        $feature_file
            .= join( "\t", qq("$type"), qq("$name"), $position ) . "\n";
    }

    my ( $result, $msg, $tracks )
        = $user_tracks->upload_data( 'My Track', $feature_file, 'text/plain',
        'overwrite' );
...
sub handle_download_userdata {
    ...
    my $resp = $self->response;
    $resp->content_type(
        $is_text ? 'text/plain' : 'application/octet-stream' );
    $resp->headers(
        {   attachment => $fname,
            charset    => $self->translate('CHARSET')
        }
...
reqed option.  The option must be placed in a [PluginName:plugin]
...
sub force_authentication {
    ...
    if ( Bio::Graphics::Browser2::Action->is_authentication_event($self->req) ) {
    ...
    my $confirm = $req->param('confirm') || $req->param('openid_confirm');
    ...
    my $output = $self->render_html_start(
        'GBrowse Login Required',
...
sub set_default_state {
    ...
    my $state = $self->state;
    $self->default_state
        if !$state->{tracks}    # always set in one form or another
            or $self->req->param('reset');
}
...
sub update_state {
    ...
    warn "[$$] update_state()" if DEBUG;
    return
        if $self->req->param('gbgff')
    ;    # don't let gbgff reqs update our coordinates!!!

#  return if url() =~ /gbrowse_img/;  # don't let gbrowse_img reqs update our coordinates either!!
    $self->_update_state;
}
...
sub _update_state {
    ...
    if ( my $seg = $self->segment ) {
     ...
     # one in whole_segment().
        ...
        $state->{view_start} ||= $seg->start
            ; # The user has selected the area that they want to see. Therefore, this
        $state->{view_stop} ||= $seg->end
            ; # will be the view_start and view_stop, rather than just start and stop.
         # asynchronous_update_coordinates will multiply this by the correct factor
         # to find the size of the segment to load

        $state->{start} ||= $seg
            ->start;    # Set regular start and stop as well, just to be safe
        $state->{stop} ||= $seg->end;    #
...
sub default_state {
    ...
    my $data_source = $self->data_source;
    %$state = ();
    @$state{ 'name', 'ref', 'start', 'stop', 'flip', 'version' }
        = ( '', '', '', '', '', 100 );
...
sub add_track_to_state {
    ...
    warn '[', Bio::Graphics::Browser2::Session->time, '] ',
        "[$$] add_track_to_state($label)"
        if DEBUG;
...
#  unshift @{$state->{tracks}},$label unless $current{$label}; # on top (better)
    ...
    warn "[$$]ADD TRACK TO STATE WAS: ",
        join ' ', grep { $state->{features}{$_}{visible} }
        sort keys %{ $state->{features} }, "\n"
        if DEBUG;
    ...
    if ( $state->{features}{$label} ) {
        ...
        $state->{features}{$label}{visible}
            = { visible => 1, options => 0, limit => 0 };
    }
...
sub create_subtrack_manager {
    ...
    my ( $dimensions, $rows, $aliases )
        = Bio::Graphics::Browser2::SubtrackTable->infer_settings_from_source(
        $source, $label )
        or return;
    ...
    my $stt = Bio::Graphics::Browser2::SubtrackTable->new(
        -columns => $dimensions,
        -rows    => $rows,
        -label   => $label,
        -key     => $key || $label,
...
sub reconfigure_track {
    ...
    ( $semantic_low, $semantic_hi ) = ( $semantic_hi, $semantic_low )
        if $semantic_low > $semantic_hi;
    $self->clip_override_ranges(
        $state->{features}{$label}{semantic_override},
        $semantic_low, $semantic_hi );

    my $o = $mode eq 'summary'
    ...
    for my $s ( grep {/^conf_/} param() ) {
        ...
        my $configured_value = $source->semantic_fallback_setting(
            $label => $s,
            $semantic_low + 1
        );
        ...
        if ( $value eq $dynamic ) {
            ...
            $o->{$s} = $value
                if !defined $configured_value
                    or $value ne $configured_value;
            if ( $glyph eq 'wiggle_whiskers' )
            {    # workarounds for whisker options
                $o->{"${s}_neg"} = $value
                    if $s =~ /^(mean_color|stdev_color)/;
                $o->{min_color} = $value if $s eq 'max_color';
            }
        }
    }
    if ( defined $o->{autoscale}
        && $o->{autoscale} =~ /local|global|chromosome/ )
    {
        undef $o->{min_score};
        undef $o->{max_score};
    }
...
sub clip_override_ranges {
    ...
    for my $r (@ranges) {
        ...
        if ( $r->[0] <= $low && $r->[1] >= $hi ) {    # case C
            $semconf->{ $r->[0] . ':' . ( $low - 1 ) } = $conf
                unless $r->[0] >= $low - 1;
            $semconf->{ ( $hi + 1 ) . ':' . $r->[1] } = $conf
                unless $hi + 1 >= $r->[1];
            $overlap++;
        }
...
sub update_options {
    ...
    if ( my @regions = shellwords( $req->param('h_region') ) ) {
        ...
        foreach (@regions) {
            ...
            $_ = "$state->{ref}:$_"
                unless /^[^:]+:-?\d/;    # add reference if not there
...
sub update_tracks {
        ...
        || ( my @ts = shellwords $req->param('ts') ) )
    {
        my @main_l
            = @ds
            ? $self->data_source->data_source_to_label(@ds)
            : $self->data_source->track_source_to_label(@ts);
        if ( !@ds && @ts ) {
            my %ds = ();
            foreach my $label (@main_l) {
                my @tracks = grep { !/^#/ }
                    shellwords $self->setting( $label => 'track source' );
                my @datasr = grep { !/^#/ }
                    shellwords $self->setting( $label => 'data source' );
        ...
        foreach my $label (@main_l) {
            my @subs = grep { !/^#/ }
                shellwords $self->setting( $label => 'select' );
            shift @subs;
...
sub update_coordinates {
    ...
    my $req = $self->req;
    if ( $req->param('ref') ) {
        $state->{ref}        = $req->param('ref');
        $state->{view_start} = $req->param('start')
            if defined $req->param('start')
                && $req->param('start') =~ /^[\d-]+/;
        $state->{view_stop} = $req->param('stop')
            if defined $req->param('stop')
                && $req->param('stop') =~ /^[\d-]+/;
        $state->{view_stop} = $req->param('end')
            if defined $req->param('end') && $req->param('end') =~ /^[\d-]+/;
        $position_updated++;
    }

    # quench uninit variable warning
    my $current_span
        = length( $state->{view_stop} || '' )
        ? ( $state->{view_stop} - $state->{view_start} + 1 )
        : 0;
    my $new_span = $req->param('span');
    ...
    elsif ( my $position_data = $req->param('overview.x') ) {
        $self->position_from_overview( $state, $position_data );
        $position_updated++;
    }
    elsif ( $position_data = $req->param('regionview.x') ) {
    ...
    if ($position_updated) {    # clip and update param
        if ( defined $state->{seg_min}
            && $state->{view_start} < $state->{seg_min} )
        {
        ...
        if ( defined $state->{seg_max}
            && $state->{view_stop} > $state->{seg_max} )
        {
    ...
    elsif ( $req->param('name') || $req->param('q') ) {
        $state->{backup_region} = [
            $state->{ref},        $state->{start}, $state->{stop},
            $state->{view_start}, $state->{view_stop}
            ]
            if $state->{ref};
        ...
        $state->{name} = $state->{search_str} = $req->param('name')
            || $req->param('q');
        $state->{dbid} = $req->param('dbid');    # get rid of this
    }
}
...
sub debug_visible {
    ...
    my $state = shift;
    warn "[$$] ", join ' ',
        map { $_ . '=>' . $state->{features}{$_}{visible} }
        @{ $state->{tracks} };
}
...
sub asynchronous_update_detail_scale_bar {
    ...
    my ( $scale_size, $scale_label )
        = $renderer->calculate_scale_size( $seg->length / $self->details_mult,
        $width / $self->details_mult );
...
sub asynchronous_update_sections {
    ...
    if ( $handle_section_name{'plugin_configure_div'} ) {
        ...
        if ($plugin_base) {
            ...
            if ($plugin) {
                ...
                $return_object->{'plugin_configure_div'}
                    = $self->translate( 'NOT_RECOGNIZED_PLUGIN',
                    $plugin_base )
                    || '' . "\n";
            }
        }
    ...
    if ( $handle_section_name{'galaxy_form'} ) {
        $return_object->{'galaxy_form'}
            = $self->galaxy_form( $self->thin_segment );
    }
    ...
    if ( $handle_section_name{'custom_tracks'} ) {
        $return_object->{'custom_tracks'}
            = $self->render_custom_track_listing();
    }

    # Community Tracks Section
    if ( $handle_section_name{'community_tracks'} ) {
        $return_object->{'community_tracks'}
            = $self->render_community_track_listing(@_)
            ;    #Passing on any search terms.
    }

    # Saved Snapshot Section
    if ( $handle_section_name{'snapshots_page'} ) {
        $return_object->{'snapshots_page'}
            = $self->snapshot_manager->render_snapshots_listing($self);
    }
...
sub asynchronous_update_element {
    ...
    if ( $element eq 'page_title' ) {
        ...
        return
            "<b>some day this will be the overview showing "
            . $self->state->{name} . "</b>";
    }
        ...
        $self->init_plugins();
        my $plugin_base = $req->param('plugin_base');
        my $plugin      = $self->plugins->plugin($plugin_base)
        ...
        if ( my $file_name = $req->param('edit_file') ) {
            return $self->edit_uploaded_file($file_name);
        }
        elsif ( $req->param('new_edit_file') ) {
...
sub asynchronous_update_coordinates {
    ...
    if ($position_updated) {    # clip and update param
        if ( defined $whole_segment_start
            ...
            && $state->{view_stop} > $whole_segment_stop )
        {
            ...
            if ( $state->{view_start} < 0 ) {
                 ...
                 ## Segment reqed is larger than the whole segment
                $state->{view_start} = $whole_segment_start;
                $state->{view_stop}  = $whole_segment_stop;
            }
        }
        ...
        unless ( defined $state->{ref} ) {
            warn
                "Reverting coordinates to last known good region (user probably hit 'back' button).";
            if ( $state->{backup_region} ) {    # last known working region
                @{$state}{ 'ref', 'start', 'stop', 'view_start', 'view_stop' }
                    = @{ $state->{backup_region} };
            }
            else {
                my $req = $self->req;
                $state->{name} = $req->param('name')
                    || $req->param('q');
                    if ( my $seg = $self->segment )
                {
                    $state->{ref}   = $seg->seq_id;
...
sub update_state_from_details_mult {
    ...
    my $start_to_load
        = int( $view_start - $length * ( $details_mult - 1 ) / 2 );
...
sub zoom {
    ...
    if (   $newstart == $state->{view_start}
        && $newstop == $state->{view_stop} )
    {
        if ( $zoom_distance < 0 ) { $newstart--; $newstop++ }
        if ( $zoom_distance > 0 ) { $newstart++; $newstop-- }
    }
    if ( $newstop - $newstart <= 2 ) {
        $newstop++;
    }    # don't go down to 2 bp level!
...
sub position_from_overview {
    ...
    my $click_position = $state->{seg_min}
        + $segment_length * ( $position_data - $padl ) / $overview_width;
...
sub update_region {
    ...
    if ( $self->setting('region segment') ) {
        $state->{region_size} = $self->req->param('region_size')
            if defined $self->req->param('region_size');
        $state->{region_size} = $self->setting('region segment')
...
sub update_section_visibility {
    ...
    for my $div ( grep {/^div_visible_/} keys %{$req->cookies} ) {
        my ($section) = $div =~ /^div_visible_(\w+)/ or next;
        my $visibility = $req->cookies->{$div};
...
sub update_galaxy_url {
    my $self  = shift;
    my $state = shift;
    if ( my $url = $self->req->param('GALAXY_URL') ) {
        warn "[$$] setting galaxy" if DEBUG;
        $state->{GALAXY_URL} = $url;
    }
    elsif ( $self->req->param('clear_galaxy') ) {
...
sub set_language {
    ...
    my $lang = Bio::Graphics::Browser2::I18n->new(
        $data_source->globals->language_path );
    my $default_language = $data_source->setting('language') || 'POSIX';

    my $accept = $self->req ? $self->req->header('Accept-Language') : '';
...
sub load_plugin_annotators {
    ...
    my %listed
        = $state->{tracks}
        ? map { $_ => 1 } @{ $state->{tracks} }
        : ();    # are we already on the list?
    my %listed_in_source = map { $_ => 1 }
        $source->configured_types;    # are we already on the list?

    for my $plugin ( $self->plugins->plugins ) {
        next unless $plugin->type eq 'annotator';
        my $name = $plugin->id
            ; # use the ID "RestrictionAnnotator" rather than the name "Restriction Annotator"
        $name = "plugin:$name";
        $source->add_type( $name, {} ) unless $listed_in_source{$name}++;
        $state->{features}{$name} ||= {
            visible => $label_visible{$name} || 0,
            options => 0,
            limit   => 0
        };
    }
...
sub detail_tracks {
    my $self             = shift;
    my $external         = $self->external_data;
    my %files_in_details = map { $_ => 1 }
        $self->featurefiles_in_section( 'detail', 'expand' );
    my @tracks = grep { $files_in_details{$_} || !/:(overview|region)$/ }
        $self->visible_tracks;
    return @tracks;
}
...
sub potential_tracks {
    ...
    my $source = $self->data_source;
    my %seenit;
    return grep { !$seenit{$_}++ && !/^_/ } (
        $source->detail_tracks, $source->overview_tracks,
        $source->plugin_tracks, $source->regionview_tracks,
    );
}
...
sub visible_tracks {
    ...
    my $state = $self->state;
    my @tracks
        = grep { $state->{features}{$_}{visible} && !/^_/ } $self->all_tracks;
    return @tracks;
}
...
sub _featurefile_sections {
  ...
  # probe for unconfigured types, which will go into detail section by default
    my @unconfigured = eval {
        grep { !$ff->type2label($_) } $ff->types;
    };
...
sub get_panel_renderer {
    ...
    return Bio::Graphics::Browser2::RenderPanels->new(
        -segment        => $seg,
        -whole_segment  => $whole,
...
sub render_detailview {
    ...
    return div(
        $self->toggle(
            'Details',
            div( { -id => 'detail_panels', -class => 'track' }, @panels )
        )
    ) . $drag_script;
}
...
sub get_detail_image_width {
    my $self        = shift;
    my $state       = shift;
    my $image_width = $state->{'width'} * $self->details_mult
        + $self->get_total_pad_width;
    return int($image_width);
}
...
sub get_total_pad_width {
    ...
    my $renderer = $self->get_panel_renderer( $self->thin_segment,
        $self->thin_whole_segment, $self->thin_region_segment, );
...
sub render_deferred {
    ...
    warn '(render_deferred(', join( ',', @$labels ), ') for section ',
        $section, ' nocache=', $nocache
        if DEBUG;

    my $renderer = $self->get_panel_renderer( $seg, $self->thin_whole_segment,
...
sub render_error_track {
    ...
    my ( $swidth, $sheight )
        = ( $font->width * length($error_message), $font->height );
...
sub render_deferred_track {
    ...
    if ( $cache->status eq 'AVAILABLE' ) {
        ...
        my $image_width
            = $track_id =~ /overview|region/
            ? $self->get_image_width( $self->state )
...
sub make_hilite_callback {
    ...
    my $state = $self->state();
    my @hiliters
        = grep { $_->type eq 'highlighter' } $self->plugins()->plugins;
    ...
    return sub {
        ...
        my %names = map { lc $_ => 1 } $feature->display_name,
...
sub categorize_track {
    ...
    if ( $user_labels->{$label} ) {
        my $cat
            = $self->user_tracks->is_mine( $user_labels->{$label} )
...
sub is_safari {
	my ($self) = @_;
    return ( $self->req->user_agent || '' ) =~ /safari/i;
}
...
sub external_data {
  ...
  # feature files and values are FeatureFile objects.
    ...
    my $too_big      = $segment
        && ( $self->get_panel_renderer($segment)->vis_length > $max_segment );
    if ( !$too_big && $segment ) {
        my $search       = $self->get_search_object;
        ...
        my $plugins      = $self->plugins;
        eval {
            $_->annotate( $meta_segment, $f, $rel2abs, $rel2abs_slow,
                $max_segment, $self->whole_segment, $self->region_segment );
...
sub add_user_tracks {
    ...
    return
        if $self
            ->is_admin; # admin user's tracks are already in main config file.
...
sub bookmark_link {
    ...
    $hm->add( 'id' => $settings->{userid} );    # slight inconsistency here
    $hm->add( 'l' => $self->join_selected_tracks );

    $hm->add( 'h_region' => $settings->{h_region} )
        if $settings->{h_region};
    my @h_feat
        = map {"$_\@$settings->{h_feat}{$_}"} keys %{ $settings->{h_feat} };
    $hm->add( 'h_feat' => \@h_feat ) if @h_feat;
    return "?" . $self->to_query_string($hm);
}
...
sub gff_dump_link {
    ...
    if ($fasta) {
        $hm->add('gbgff' => 'Fasta' );
        $hm->add('m' => 'application/x-fasta' );
    }
    else {
        $hm->add('gbgff' => 'Save' );
        $hm->add('m' => 'application/x-gff3' );
    }
...
sub galaxy_link {
    ...
    my $submit_it
        = q(document.galaxyform.submit());    #}) - Syntax highlight fixing.
    return "$clear_it;$submit_it";
}

sub galaxy_clear {
    return
        q(new Ajax.Request(document.URL,{method:'post',postBody:'clear_galaxy=1'}));
}
...
sub image_link {
    ...
    my $flip     = $settings->{flip} || $req->param('flip') || 0;
    ...
    my $width  = $req->param('view_width') || $settings->{width};
    my $start  = $req->param('view_start') || $settings->{view_start};
    my $stop   = $req->param('view_stop') || $settings->{view_stop};
    $stop++;
    my $name     = "$settings->{ref}:$start..$stop";
    my $selected = $self->join_selected_tracks;
    my $options  = join '+',
        map { join '+', uri_escape($_), $settings->{features}{$_}{options} }
        map { /\s/ ? "$_" : $_ }
        grep { $settings->{features}{$_}{options} } @$tracks;
...
sub fcgi_req {
    ...
    my $req = FCGI::Request( \*STDIN, \*STDOUT, \*STDERR, \%ENV, 0,
        FCGI::FAIL_ACCEPT_ON_INTR() );
    return $FCGI_REQUEST = ( $req && $req->IsFastCGI ? $req : 0 );
}
...
sub prepare_fcgi_for_fork {
    my $self  = shift;
    my $state = shift;
    my $req   = $self->fcgi_req() or return;
...
sub chrom_sizes {
    ...
    my $name   = $source->name;
    my $sizes  = File::Spec->catfile( $self->globals->tmpdir('chrom_sizes'),
        "$name.sizes" );
    ...
    if ( my @seqids = eval { $db->seq_ids } ) {
        for (@seqids) {
            my $segment = $db->segment($_)
                or die "Can't find chromosome $_ in default database";
...
sub change_ids {
    ...
    my $new_userid    = shift;
    my $usertracks    = $self->user_tracks;
    $usertracks->change_ids( $old_uploadsid, $new_uploadsid, $old_userid,
        $new_userid );
    my $userdb = $self->{userdb};
    $userdb->change_ids( $old_uploadsid, $new_uploadsid, $old_userid,
        $new_userid );
}
...
sub render_toggle_external_table {
    my $self = shift;
    croak
        "render_toggle_external_table() should not be called in parent class";
}

sub render_toggle_userdata_table {
    my $self = shift;
    croak
        "render_toggle_userdata_table() should not be called in parent class";
}
...
sub fatal_error {
    ...
    my @msg  = @_;
    croak
        'Please call fatal_error() for a subclass of Bio::Graphics::Browser2::Render';
}

sub zoomBar {
    my $self = shift;
    croak
        'Please define zoomBar() in a subclass of Bio::Graphics::Browser2::Render';
}
...
sub login_manager {
    ...
    eval "require Bio::Graphics::Browser2::Render::Login"
        unless Bio::Graphics::Browser2::Render::Login->can('new');
    return $self->{login_manager}
        = Bio::Graphics::Browser2::Render::Login->new($self);
}

sub snapshot_manager {
    my $self = shift;
    return $self->{snapshot_manager}
        ||= Bio::Graphics::Browser2::Render::SnapshotManager->new($self);
}
...
sub feature_summary_message {
    ...
    my $message = $self->data_source->setting( $label => 'key' ) . ' '
        . lc( $self->tr('FEATURE_SUMMARY') );
...
sub feature_interaction {
    ...
    if ($tips) {
        ...
        my ( $balloon_style, $balloon_action )
            = $renderer->balloon_tip_setting(
            $event_type eq 'mousedown' ? 'balloon click' : 'balloon hover',
            $label, $feature, undef, undef );
        $balloon_action
            ||= $renderer->make_title( $feature, undef, $label, undef )
            if $source->global_setting('titles are balloons')
                && $event_type eq 'mouseover';
        $balloon_style ||= 'GBubble';
        if ($balloon_action) {
            my $action
                = $balloon_action =~ /^(http|ftp):/
    ...
    my $target
        = $renderer->make_link_target( $feature, undef, $label, undef );
    return ( 'text/plain',
        $target
