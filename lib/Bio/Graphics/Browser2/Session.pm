...
sub new {
    ...
    my ( $driver, $id, $session_args, $default_source, $lockdir, $locktype,
        $expire_time, $mode )
        = @args{
        'driver',  'id',       'args',    'source',
        'lockdir', 'locktype', 'expires', 'mode'
        };
    ...
    $self->{session}
        = $mode eq 'exclusive' ? $self->lock_ex($id) : $self->lock_sh($id);
    warn "CGI::Session->new($driver,$id,$session_args)=>",
        $self->{session}->id
        if DEBUG;
    ...
    if (DEBUG_LOCK) {
        ...
        my @tracks = sort grep { $state->{features}{$_}{visible} }
            keys %{ $state->{features} };
...
sub locktype {
    ...
    return 'mysql'
        if $self->{locktype} =~ /^(dbi:mysql|mysql):/ && $HAS_MYSQL;
...
sub lock {
    ...
    if ($@) {
        ...
        warn(
            "[$$] session lock timed out on request after $elapsed\n",
            CGI::request_method(), ': ', CGI::url( -path => 1 ),
            ' ', CGI::query_string()
        );
    }
    else {
        warn '[', $self->time, '] ',
            "[$$] ...$type lock obtained after $elapsed"
            if DEBUG_LOCK;
    }
...
sub unlock {
    ...
    $self->lockobj(undef);
    warn "[$$] lock released after ",
        sprintf( "%2.1f", $self->time() - $self->locktime ), " s"
        if DEBUG;
}
...
sub lockfile {
    ...
    my $id   = shift;
    my ($a) = $id =~ /^(.{2})/;
    return ( File::Spec->catfile( $self->{lockdir}, $a ), $id );
}
...
sub modified {
    ...
    $session->param($source => $config_hash);
}
...
sub set_nonce {
    ...
    if ($remember) {
        $self->{session}->expire( '.nonce' => '30d' )
            ;    # remembers authorization for 30 days
        $self->remember_auth(1);
    }
    else {
        $self->{session}->expire( '.nonce' => '1d' )
            ;    # force reauthorization every day
