...
sub username {
    my $self = shift;
    return $self->{'.authenticated_username'} || $self->req->user;
}
...
sub authorized {
    ...
    my $host = $self->req->remote_host;
    my $addr = $self->req->address;
    ...
    my @tokens
        = split
        /\s*(satisfy|order|allow from|deny from|require user|require group|require valid-user)\s*/i,
        $restrict;
    ...
    while (@tokens) {
        ...
        if ( $directive eq 'require group' && defined $user ) {
            ...
            if ( my $auth_plugin = $self->authenticator ) {
                for my $grp (@values) {
                    $users{$user}
                        ||= $auth_plugin->user_in_group( $user, $grp );
                }
            }
            else {
                warn
                    "To use the 'require group' limit you must load an authentication plugin. Otherwise use a subroutine to implement role-based authentication.";
            }
        }
    }

    my $allow
        = $mode eq 'allow,deny' ? match_host( \@allow, $host, $addr )
        && !match_host( \@deny, $host, $addr )
        : 'deny,allow' ? !match_host( \@deny, $host, $addr )
        || match_host( \@allow, $host, $addr )
...
sub match_host {
    ...
    for my $candidate (@$matches) {
        if ( $candidate eq 'all' ) {
            ...
            $addr .= '.'
                unless $addr
                    =~ /\.$/;    # these lines ensure subnets match correctly
            ...
            $candidate = ".$candidate"
                unless $candidate
                    =~ /^\./;    # these lines ensure domains match correctly
