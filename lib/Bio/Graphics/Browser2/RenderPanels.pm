...
sub request_panels {
    ...
    my ( $local_labels, $remote_labels )
        = $self->sort_local_remote($data_destinations);

    warn
        "[$$] request_panels(): section = $args->{section}; local labels = @$local_labels, remote labels = @$remote_labels"
        if DEBUG;
...
# Bio::Graphics::Browser2::DataBase->clone_databases();
    ...
    if ( $args->{deferred} ) {
        ...
        if ($child) {
            warn "[$$] Forked new rendering panel $child for $args->{section}"
                if DEBUG;
...
sub render_track_images {
    ...
    while (%still_pending) {
        ...
        $delay *= $k
            ;   # sleep a little longer each time using an exponential backoff
    }
    return \%results;
}
...
sub make_requests {
    ...
    foreach my $label ( @{ $labels || [] } ) {
        ...
        if ( $feature_files && exists $feature_files->{$track} ) {
            ...
            unless ( ref $feature_file ) {    # upload problem!
                my $cache_object = Bio::Graphics::Browser2::CachedTrack->new(
                    ...
                    -extra_args => [
                        @cache_extra,      @filter_args,
                        @featurefile_args, @subtrack_args,
                        $format_option,    $label
                    ],
                );

                my $msg = eval { $args->{remotes}->error($track) };
                $cache_object->flag_error( $msg
                        || "Could not fetch data for $track" );
            ...
            @featurefile_args = eval {
                       $feature_file->isa('Bio::Das::Segment')
                    || $feature_file->types, $feature_file->mtime;
            };
        }

        warn
            "[$$] creating CachedTrack for $label, nocache = $args->{nocache}"
            if DEBUG;
        my $cache_time
            = $args->{nocache}   ? -1
        ...
        my $cache_object = Bio::Graphics::Browser2::CachedTrack->new(
            ...
            -track_args => \@track_args,
            -extra_args => [
                @cache_extra,   @filter_args,   @featurefile_args,
                @subtrack_args, $format_option, $label
            ],
...
sub use_renderfarm {
    ...
    $LPU_AVAILABLE = eval { require LWP::UserAgent; }
        unless defined $LPU_AVAILABLE;
    $STO_AVAILABLE = eval { require Storable; 1; }
        unless defined $STO_AVAILABLE;
    $Storable::Deparse = 1;

    $self->{use_renderfarm} = $LPU_AVAILABLE && $STO_AVAILABLE;
    return $self->{use_renderfarm} if $self->{use_renderfarm};
    warn
        "The renderfarm setting requires the LWP::UserAgent and Storable modules,
...
sub drag_and_drop {
    ...
    my $source = $self->source;
    return
        unless $source->global_setting('drag and drop')
    ;    # drag and drop turned off
    return
        if $source->global_setting('postgrid')
    ;    # postgrid forces drag and drop off
    1;
}
...
sub wrap_rendered_track {
    ...
    my $add_or_remove = $self->language->translate('ADDED_TO')
...
# on all EVEN reloads of the element by ajax calls. Weird.
    ...
    $configure_this_track
        .= $self->language->translate('CONFIGURE_THIS_TRACK')
    ...
    my $about_this_track = '';
    $about_this_track
        .= $self->language->translate( 'ABOUT_THIS_TRACK', $label )
    ...
    if ( $label =~ /^plugin:/ ) {
        ...
        $config_click = "GBox.showTooltip(event,'$config_url',true)";
    }
    ...
    my $download_click
        = "GBox.showTooltip(event,'url:?action=download_track_menu;track=$escaped_label;view_start='+TrackPan.get_start()+';view_stop='+TrackPan.get_stop(),true)"
        unless $label =~ /^(http|ftp)/;
    ...
    my @images = (
        ...
        ? img(
            {   -src         => $starIcon,
                ...
                $self->if_not_ipad(
                    -onMouseOver =>
                        "$balloon_style.showTooltip(event,'$add_or_remove')"
                ),
            }
            )
        : '',
        img({   -src     => $icon,
                ...
                $self->if_not_ipad(
                    -onMouseOver =>
                        "$balloon_style.showTooltip(event,'$show_or_hide')"
                ),
            }
        ),
        ...
        img({   -src     => $kill,
                ...
                $self->if_not_ipad(
                    -onMouseOver =>
                        "$balloon_style.showTooltip(event,'$kill_this_track')"
                ),
            }
        ),
        img({   -src   => $share,
    ...
    my $menuicon = img(
        ...
        my $popmenu = div(
        { -id => "popmenu_${title}", -style => 'display:none' },
        ...
        div({   -class        => 'ipadcollapsed',
            ...
            div( { -class => 'linkbg', }, $configure_ipad )
        ),
        div({   -class       => 'ipadcollapsed',
                -onmousedown => $fav_click,
            },
            div({   -class   => 'linkbg',
                    -onClick => "swap(this,'Favorite','Unfavorite')"
                },
                $bookmark
            )
        ),
    ...
    my $titlebar = span(
        ...
        $self->if_ipad(
            span(
                {   -class => 'menuclick',
                    -onClick =>
                        "GBox.showTooltip(event,'load:popmenu_${title}')"
                },
                $menuicon,
            ),
        ),
    ...
    my $pad = $self->render_image_pad(
        $args{section}
            || Bio::Graphics::Browser2::DataSource->get_section_from_label(
            $label),
    );
    ...
    if ( $is_scalebar && $is_detail ) {
        my $style
            = 'opacity:0.35;position:absolute;border:none;cursor:pointer';
        my $pan_left = img(
        ...
        $overlay_div = div(
            {   -id    => "${label}_overlay_div",
                -style => "position:absolute; top:0px; width:100%; left:0px",
            },
            $pan_left . $pan_right . $scale_div
        );
    }

    my $inner_div = div( { -id => "${label}_inner_div" }, $img . $pad_img )
    ...
    my $subtrack_labels = join '', map {
        ...
        div({   -class => 'subtrack',
                -style => "top:${top}px;left:${left}px;background-color:white"
            },
            $label
        );
    } @$titles;
    ...
    my $html = div(
        {   -class => 'centered_block',
            -style => "position:relative;overflow:hidden"
        },
        ( $show_titlebar ? $titlebar : '' ) 
            . $popmenu
            . $subtrack_labels
            . $inner_div
            . $overlay_div
...
sub run_remote_requests {
    ...
    eval { use HTTP::Request::Common; }
        unless HTTP::Request::Common->can('POST');
    ...
    $args{$_} = $args->{$_}
        foreach ( 'section', 'image_class', 'cache_extra' );
    ...
    my $timeout 
        = $source->global_setting('slave_timeout')
    ...
    FETCH: {
            ...
            warn "$url=>@labels: ", $response->status_line, " ($elapsed s)"
                if DEBUG;
            ...
            if ( $response->is_success ) {
                ...
                for my $label ( keys %$contents ) {
                    my $map = $contents->{$label}{map}
                        or die
                        "Expected a map from remote server, but got nothing!";
                    my $titles = $contents->{$label}{titles}
                        or die
                        "Expected titles from remote server, but got nothing!";
                    my $gd2 = $contents->{$label}{imagedata}
                        or die
                        "Expected imagedata from remote server, but got nothing!";
...
# right if all of the tracks there are multiple equivalent slaves for the tracks
                my %urls = map { $_ => 1 }
                    map { shellwords( $source->remote_renderer ) } @labels;
                ...
                $requests->{$_}->flag_error($response_line)
                    foreach keys %{ $renderers{$uri} };
            }
        }
...
sub render_scale_bar {
    ...
    $add_track_extra_args{'-postgrid'} = $args{'postgrid'}
        if $args{'postgrid'};
    ...
    my $width
        = ( $section eq 'detail' )
        ? $self->render->get_detail_image_width($state)
        : $self->render->get_image_width($state);
...
sub render_image_pad {
    ...
    $segment ||=
          $section eq 'overview' ? $self->whole_segment
    ...
    my @track_args = ();
    my @extra_args = ( $self->settings->{start}, $self->settings->{stop} );
...
sub bump_density {
    ...
    my $conf = $self->source;
    my $bd 
        = $conf->global_setting('bump density')
...
sub label_density {
    ...
    my $conf = $self->source;
    my $ld 
        = $conf->global_setting('label density')
...
sub log10 {
    return eval { log(shift) / log(10) } || 0;
}
...
sub make_scale_feature {
    ...
    my ( $guesstimate, $label )
        = $self->calculate_scale_size( $length, $width );
    ...
    my $start
        = int( ( $segment->start + $segment->end ) / 2 - $guesstimate / 2 );
...
sub make_map {
    ...
    push @map,
        $self->make_centering_map( shift @$boxes,
        $flip, 0, $first_box_is_scale )
    ...
    if ($inline) {
        $inline_options = {
            tips => $source->global_setting('balloon tips')
                && $settings->{'show_tooltips'},
            summary =>
                $source->show_summary( $label, $length, $self->settings ),
            use_titles_for_balloons =>
                $source->global_setting('titles are balloons'),
            balloon_style => $source->global_setting('balloon style')
                || 'GBubble',
            balloon_sticky => $source->semantic_fallback_setting(
                $label, 'balloon sticky', $length
            ),
            balloon_height =>
                $source->semantic_fallback_setting( $label, 'balloon height',
                $length )
                || 300,
        };
    }
    ...
    foreach my $box (@$boxes) {
        ...
        my $attributes
            = $inline
            ? $self->make_imagemap_element_inline( $feature, $panel, $label,
            $box->[5], $inline_options )
            : $self->make_imagemap_element_callback( $feature, $track_dbid );
        $attributes or next;
        my $fname
            = eval  { $feature->display_name }
            || eval { $box->[0]->name }
            || 'unnamed';
        ...
        for my $att ( keys %$attributes ) {
            next
                unless defined $attributes->{$att}
                    && length $attributes->{$att};
...
sub make_imagemap_element_inline {
    ...
    if ($summary) {
        return {
            onmouseover =>
                $self->render->feature_summary_message( 'mouseover', $label ),
            onmouseeown =>
                $self->render->feature_summary_message( 'mousedown', $label ),
            href   => 'javascript:void(0)',
            inline => 1
        };
    }
    ...
    my $title
        = unescape( $self->make_title( $feature, $panel, $label, $track ) );
    ...
    if ($tips) {
        ...
        my ( $balloon_ht, $balloonhover )
            = $self->balloon_tip_setting( 'balloon hover',
            $label, $feature, $panel, $track, 'inline' );
        my ( $balloon_ct, $balloonclick )
            = $self->balloon_tip_setting( 'balloon click',
            $label, $feature, $panel, $track, 'inline' );
...
sub make_centering_map {
    ...
    if (   $scale_map_type eq 'interval_map'
        && $length / RULER_INTERVALS > $self->get_max_segment )
    {
    ...
    for my $i ( 0 .. $ruler_intervals - 1 ) {
        ...
        if ( $scale_map_type eq 'centering_map' ) {
         ...
         # put the middle of the sequence range into the middle of the picture
            ...
            $start
                = int( $flip ? $end - $scale * $x1 : $offset + $scale * $x1 );
...
sub run_local_requests {
    my $self     = shift;
    my $requests = shift
        ;   # a hash of labels => Bio::Graphics::Browser2::CachedTrack objects
    my $args   = shift;
    my $labels = shift;
...
#
            ...
            if (my $hide = $source->semantic_setting(
                    $label => 'hide',
                    $self->segment_length
                )
                )
            {
                ...
                if ( exists $feature_files->{$base} ) {
                    ...
                    warn "track_setup($label): ", time() - $time, " seconds "
                        if BENCHMARK;
                    ...
                    warn "add_features($label): ", time() - $time, " seconds "
                        if BENCHMARK;
                ...
                $gd = $panel->gd;
                warn "render gd($label): ", time() - $time, " seconds "
                    if BENCHMARK;
                ...
                warn "render titles($label): ", time() - $time, " seconds "
                    if BENCHMARK;

                my $boxes = $panel->boxes;
                warn "boxes($label): ", time() - $time, " seconds "
                    if BENCHMARK;

                $map = $self->make_map( $boxes, $panel, $label, \%trackmap,
                    0 );
                warn "make_map($label): ", time() - $time, " seconds "
                    if BENCHMARK;
            }
        ...
        if ($@) {
            ...
            if ( $@ =~ /timeout/ ) {
                $requests->{$label}->flag_error(
                    'Timeout; Try turning off tracks or looking at a smaller region.'
                );
            }
            else {
                $requests->{$label}->flag_error($@);
...
sub render_hidden_track {
    ...
    $gd->string( GD->gdMediumBoldFont, ( $wid - $len ) / 2,
        0, $message, $black );
    return $gd;
}
...
sub select_features_menu {
    ...
    my $escaped_label = CGI::escape($label);
    my $subtrack_over
        = "GBubble.showTooltip(event,'url:?action=show_subtracks;track=$escaped_label',false)";
    my $subtrack_click
        = "GBox.showTooltip(event,'url:?action=select_subtracks;track=$escaped_label',true)";

    # modify the title to show that some subtracks are hidden
    $$titleref .= " "
        . span(
        {   -class       => 'clickable',
            -onMouseOver => "GBubble.showTooltip(event,'"
                . $self->language->translate('CLICK_MODIFY_SUBTRACK_SEL')
                . "')",
...
sub add_features_to_track {
    ...
    for my $db ( keys %db2db ) {
        ...
        warn "[$$] RenderPanels->get_summary_iterator(@summary_types)"
            if DEBUG;
        if (@summary_types
            && (my $iterator = $self->get_summary_iterator(
                    $db2db{$db}, $segment, \@summary_types
                )
            )
            )
        {
        ...
        if (@full_types
            && ( my $iterator
                = $self->get_iterator( $db2db{$db}, $segment, \@full_types ) )
            )
        {
            $iterators{$iterator}     = $iterator;
            $iterator2dbid{$iterator} = $source->db2id($db);
   ...
   # bit. This may not be worth the effort.
    ...
    while ( keys %iterators ) {
        for my $iterator ( values %iterators ) {
            ...
            $source->add_dbid_to_feature( $feature,
                $iterator2dbid{$iterator} );
            my @labels = $self->feature2label($feature);

            warn
                "[$$] $iterator->next_seq() returns $feature, will assign to @labels"
                if DEBUG;
...
# Handle name-based groupings.
                unless ( exists $group_pattern{$l} ) {
                    $group_pattern{$l} = $source->semantic_setting(
                        $l => 'group_pattern',
                        $length
                    );
                    $group_pattern{$l} =~ s!^/(.+)/$!$1!
                ...
                if (  !$is_summary
                    && $stt
                    && (defined(
                            my $id = $stt->feature_to_id_sub->($feature)
                        )
                    )
                    )
                {
                    $groups{$l}{$id} ||= Bio::Graphics::Feature->new(
                        -type       => 'group',
                        -primary_id => $id,
...
# the latter is traditional name-based grouping based on a common prefix/suffix
    ...
    for my $l ( keys %groups ) {
        ...
        if ($has_subtracks{$l}
            && !$source->semantic_setting(
                $l => 'hide empty subtracks',
                $length
            )
            )
        {
        ...
        if ( $source->setting( $l => 'subtrack single plot' ) ) {
            my $tempFeature = Bio::Graphics::Feature->new(
                -type   => 'group',
                -start  => $segment->start,
                -end    => $segment->end,
                -seq_id => $segment->seq_id,
            );
            my $tempSubFeature = Bio::Graphics::Feature->new(
                -type   => 'scale',
                -scale  => 'three',
                -name   => 'scales',
                -start  => $segment->start,
            ...
            for my $f ( values %$g ) {
                my @colours
                    = (
                    qw(aqua black blue fuchsia gray green lime maroon navy olive purple red silver teal yellow)
                    );
    ...
    for my $l ( keys %$tracks ) {
        ...
        my $do_bump
            = $self->do_bump( $l, $pack_options, $count, $max_bump, $length );

        my $do_label
            = $self->do_label( $l, $pack_options, $count, $max_labels,
            $length );

        my $do_description
            = $self->do_description( $l, $pack_options, $count, $max_labels,
            $length );

        $tracks->{$l}->configure(
            -bump => (
                $has_subtracks{$l}
                    && ( $source->setting( $l => 'subtrack single plot' ) )
                ) ? 0 : $do_bump,
            -label       => $do_label,
        ...
        if ( eval { $tracks->{$l}->features_clipped } )
        {    # may not be present in older Bio::Graphics
            my $max   = $tracks->{$l}->feature_limit;
            my $count = $tracks->{$l}->feature_count;
            my $message
                = $count == $self->source->globals->max_features
                ? 'FEATURES_CLIPPED_MAX'
                : 'FEATURES_CLIPPED';
            $tracks->{$l}->panel->key_style('between');
            $tracks->{$l}->configure(
                -key => $self->language->translate( $message, $max, $count )
            );
        }
    }
...
sub get_iterator {
    ...
    if ( $db->can('get_seq_stream') ) {
        ...
        push @args, ( -max_features => $max )
            if $max > 0;    # some adaptors allow this
    ...
    if ( eval { $segment->factory || '' eq $db } ) {
        ...
        ($db_segment)
            = $db->segment( $segment->seq_id, $segment->start,
            $segment->end );
    }
...
sub add_feature_file {
    ...
    eval {
        $file->render( $args{panel}, $args{position}, $options->{$name},
            $self->bump_density, $self->label_density, $select,
            $self->segment, );
    };
...
sub create_panel_args {
    ...
    my ( $seg_start, $seg_stop, $flip )
        = $self->segment_coordinates( $segment, $args->{flip} );
    ...
    my @argv = (
        ...
        -gridmajorcolor => $source->global_setting('grid major color')
            || 'cyan',
        @pass_thru_args
        ,    # position is important here to allow user to override settings
    );
...
sub image_padding {
    ...
    my $source = $self->source;
    return
        defined $source->global_setting('image_padding')
...
sub create_track_args {
    ...
    my $semantic_override
        = Bio::Graphics::Browser2::Render->find_override_region(
        $state->{features}{$label}{semantic_override}, $length );
    my $override
        = $is_summary ? $state->{features}{$label}{summary_override}
        : $semantic_override
    ...
    push @override, ( -feature_limit => $override->{limit} )
        if $override->{limit};
    push @override, ( -record_label_positions => 0 )
        unless $args->{section} && $args->{section} eq 'detail';

    my @summary_args = ();
    if ($is_summary) {
        @summary_args
            = $source->Bio::Graphics::FeatureFile::setting("$label:summary")
    ...
    if ( $self->subtrack_manager($label) ) {
        ...
        my $left_label
            = $source->semantic_setting( $label => 'label_position', $length )
            || '' eq 'left';
        ...
        my $group_label
            = $source->semantic_setting( $label => 'glyph', $length )
            !~ /xyplot|wiggle|density|whisker/;
        ...
        push @default_args,
            ...
            -group_subtracks =>
                $source->setting( $label => 'subtrack single plot' ) ? 0 : 1,
            );
    }
    ...
    if ( $source->semantic_setting( $label => 'global feature', $length ) ) {
        eval {    # honor the database indicated in the track config
            ...
            my $class
                = eval { $segment->seq_id->class } || eval { $db->refclass };
            ($segment) = $db->segment(
                -name  => $segment->seq_id,
                -start => $segment->start,
                -end   => $segment->end,
        ...
        warn $@ if $@;
        @args = (
            $segment, @default_args, $source->default_style,
            $source->i18n_style( $label, $lang ),
            @summary_args, @override,
        );
    }
    else {
        @args = (
            @default_args, $source->default_style,
...
sub do_bump {
    ...
    my $conf_bump
        = $source->semantic_setting( $track_name => 'bump', $length );
    $option ||= 0;
    return
          defined $conf_bump ? $conf_bump
        : $option == 0       ? $maxed_out
        : $option == 1       ? 0
        : $option == 2       ? 1
        : $option == 3       ? 1
...
sub do_label {
    ...
    my $conf_label
        = $source->semantic_setting( $track_name => 'label', $length );
    $conf_label = 1 unless defined $conf_label;

    $option ||= 0;
    return
          $option == 0 ? $maxed_out && $conf_label
...
sub do_description {
    ...
    my $conf_description
        = $source->semantic_setting( $track_name => 'description', $length );
    $conf_description = 0 unless defined $conf_description;
    $option ||= 0;
    return
          $option == 0 ? $maxed_out && $conf_description
...
sub feature2label {
    ...
    my $type
        = eval  { $feature->type }
        || eval { $feature->source_tag }
        || eval { $feature->primary_tag }
        or return;
    ( my $basetype = $type ) =~ s/:.+$//;
...
sub make_link {
    ...
    $link = $feature->make_link
        if $feature->can('make_link') && !defined $link;

    # general defaults
    $link = $data_source->code_setting( 'TRACK DEFAULTS' => 'link' )
        unless defined $link;
    $link = $data_source->code_setting( general => 'link' )
        unless defined $link;
    $link = $data_source->globals->setting( general => 'link' )
        unless defined $link;
    ...
    if ( ref($link) eq 'CODE' ) {
        ...
        my $dbid = eval { $feature->gbrowse_dbid }
            || ( $data_source->db_settings($label) )[0];
...
TRY: {
        if ( $label && eval { $label->isa('Bio::Graphics::FeatureFile') } ) {
            ...
            $key =~ s/s$//;
            $key = "source = " . $feature->segment->dsn
                if $feature->isa('Bio::Das::Feature');    # for DAS sources

            my $length = $self->segment_length($label);

            my $link = $source->semantic_fallback_setting( $label, 'title',
                $length );
            ...
            return $source->link_pattern( $link, $feature )
                if defined $link && $link ne 'AUTO';
        }
    }
...
sub segment_length {
    ...
    my $section
        ...
        : 'detail';
    return eval {
              $section eq 'detail'   ? $self->segment->length
...
sub make_link_target {
    ...
    my $link_target 
        = $source->code_setting( $label, 'link_target' )
...
sub balloon_tip_setting {
    ...
    $value = $source->code_setting( 'TRACK DEFAULTS' => $option )
        unless defined $value;
    $value = $source->code_setting( 'general' => $option )
        unless defined $value;
    ...
    return sub {
        ...
        for my $r (@h_regions) {
            ...
            if ( $fgcolor && $fgcolor ne 'none' ) {
                ...
                $gd->setStyle( $c, $c, gdTransparent, gdTransparent )
