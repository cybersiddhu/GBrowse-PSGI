...
sub new {
    ...
    my ( $name, $description, $globals , $req) = @_;
    ...
    my $self
        = $class->can('new_from_cache')
        && !$ENV{GBROWSE_NOCACHE} && ( $config_file_path !~ /\|$/ )
...
sub precompile_glyphs {
    ...
    for my $l ( $self->labels ) {
        ...
        eval "use Bio::Graphics::Glyph::$glyph"
            unless "Bio::Graphics::Glyph\:\:$glyph"->can('new');
    }
}
...
sub details_multiplier {
    ...
    $value = 25
        if ( $value > 25 )
        ; #set upper limit for performance reasons (prevent massive image files)
    ...
    if (   ( $max_length > 0 )
        && ( $request_length > 0 )
        && ( ( $request_length * $value ) > $max_length ) )
    {
        return $max_length
            / $request_length
            ; #limit details multiplier so that region does not go out of bounds
    }
...
sub unit_label {
    ...
    my $label;
    $label
        = $unit =~ /^[kcM]/ ? sprintf( "%.4g %s", $value, $unit )
...
sub karyotype_tracks {
    my $self = shift;
    grep { ( $_ eq 'karyotype' || /:karyotype$/ ) && $self->authorized($_) }
        $self->configured_types;
}

sub plugin_tracks {
    my $self = shift;
    grep { ( $_ eq 'plugin' || m!plugin:! ) && $self->authorized($_) }
        $self->configured_types;
}
...
sub label2type {
    ...
    return shellwords( $self->setting( $l, 'feature' )
            || $self->setting( $label, 'feature' )
            || '' );
}

sub track_listing_class {
    my $self  = shift;
    my $style = lc $self->global_setting('track listing style')
        || 'categories';
    my $subclass
        = $style eq 'categories' ? 'Categories'
        : $style eq 'facets'     ? 'Facets'
        :                          die "invalid track listing style '$style'";
    return 'Bio::Graphics::Browser2::Render::HTML::TrackListing::'
        . $subclass;
}
...
sub i18n_style {
    ...
    my %lang_options = map { $_->[1] => $options{ $_->[0] } }
        sort { $b->[2] <=> $a->[2] }
        map {
        my ( $option, undef, $lang ) = /^(-[^:]+)(:(\w+))?$/;
        [ $_ => $option, $priority{ $lang || '' } || 99 ]
        }
        keys %options;
    %lang_options;
}
...
sub show_summary {
    ...
    $settings ||= {};
    return 0
        if defined $settings->{features}{$label}{summary_mode_len}
            && !$settings->{features}{$label}{summary_mode_len};
    my $c = $settings->{features}{$label}{summary_mode_len}
        || $self->semantic_fallback_setting( $label => 'show summary',
        $length );
    ...
    return 0
        if $class->isa('Bio::Graphics::Glyph::xyplot')
            or $class->isa('Bio::Graphics::Glyph::minmax');
    return 0
        if $self->semantic_fallback_setting(
...
sub can_summarize {
    ...
    return
        if $g =~ /wiggle|xyplot|density/; # don't summarize wiggles or xyplots
...
# 2. a section like "Gene" which has no cutoff to use.
    if (my @lowres = map { [ split ':' ] }
        grep { /^$label:(\d+)/ && $1 <= $length } $self->configured_types
        )
    {
...
sub show_section
{ # one of instructions, upload_tracks, search, overview, region, detail, tracks, or display_settings
    my $self    = shift;
...
sub get_ranges {
    ...
    my $rangestr = $self->global_setting('zoom levels')
...
sub type2label {
    ...
    for my $i ( $main_inverted, $user_inverted ) {
        ...
        $labels{$_}++
            foreach map { keys %{ $i->{$type}{$dbid}{$_} } } @lengths;
    }
...
sub invert_types {
    ...
    $master_cache
        or die
        "no bio::graphics cache file found at $master_cache? Shouldn't happen..";
    ...
    for my $label ( sort keys %{$config} ) {
        ...
        my $feature = $self->semantic_setting( $label => 'feature', $length )
            or next;
...
sub default_labels {
    ...
    $defaults ||= $self->setting( 'general' => 'default features' )
...
sub make_link {
    croak
        "Do not call make_link() on the DataSource. Call it on the Render object";
}
...
sub databases {
    my $self = shift;
    my @dbs = map { s/:database//; $_ }
        grep {/:database$/} $self->configured_types;
    return @dbs;
}
...
sub db_settings {
    ...
    if ( $track =~ /(.+):(\d+)$/ )
    {    # in the case that we get called with foo:499
        $track  = $1;
        $length = $2;
    }
    ...
    return @{ $DB_SETTINGS{ $self, $semantic_label } }
        if defined $DB_SETTINGS{ $self, $semantic_label };
    ...
    if ( $track =~ /:database$/ ) {
        ...
        = $self->semantic_fallback_setting( $track => 'database', $length ) )
    {
        ...
        $self->semantic_fallback_setting( $track => 'db_adaptor', $length ) )
    {
    ...
    if ( defined $DB_SETTINGS{ $self, $symbolic_db_name } ) {
        return @{ $DB_SETTINGS{ $self, $semantic_label }
                ||= $DB_SETTINGS{ $self, $symbolic_db_name } };
    }

    my $adaptor = $self->semantic_setting( $symbolic_db_name => 'db_adaptor',
        $length );
    unless ($adaptor) {
    ...
    my $args = $self->semantic_fallback_setting(
        $symbolic_db_name => 'db_args',
        $length
    );
    my @argv
        = ref $args eq 'CODE'
...
# Do environment substitutions in the args. Assume that the environment is safe.
    ...
    if (defined(
            my $a = $self->semantic_setting(
                $symbolic_db_name => 'aggregators',
                $length
            )
        )
        )
...
sub open_database {
    ...
    my $db
        = Bio::Graphics::Browser2::DataBase->open_database( $adaptor, @argv );
    $self->fix_sqlite
        if $adaptor eq 'Bio::DB::SeqFeature::Store'
            && "@argv" =~ /SQLite/;    # work around broken bioperl
    ...
    unless ( $self->{databases_seen}{$db}++ ) {
        ...
        unless ( $track eq 'general' ) {
            my $default = $self->open_database()
                ;                      # I hope we don't get into a loop here
...
sub search_options {
    ...
    my $dbid = shift;
    return
           $self->setting( $dbid => 'search options' )
...
sub dbs {
    ...
    my @anonymous
        = grep { !$named{$_} && $self->setting( $_ => 'db_adaptor' ) }
        @labels;
...
sub generate_image {
    ...
    $self_name =~ s/\\/_/g
        ;    # Because Safari and Chrome translate backslash to forward slash
    my $path = $self->globals->tmpimage_dir($self_name);

    my $image_url = $self->globals->image_url;
    my $url       = sprintf( "%s/%s/%s.%s",
        $image_url, $self_name, $signature, $extension );
...
sub _secondary_key_to_label {
    ...
    if ( !exists $index->{$field} ) {
        for my $label ( $self->labels ) {
            my @sources = shellwords $self->setting( $label => $field )
                or next;
...
sub add_user_type {
    ...
    my $cc
        = ( $type =~ /^(general|default)$/i ) ? 'general' : $type; # normalize
...
sub usertype2label {
    ...
    for my $label (@userlabels) {
        ...
        my @types
            = shellwords( $self->{_user_tracks}{config}{$label}{feature} );
...
sub fix_sqlite {
    ...
    no warnings;
    *Bio::DB::SeqFeature::Store::DBI::SQLite::_has_spatial_index
        = sub {return};
}
...
sub use_inline_imagemap {
    my $self = shift;
    my ( $label, $length ) = @_;
    my $inline = $self->semantic_fallback_setting(
        $label => 'inline imagemaps',
        $length
        )
        || $self->semantic_fallback_setting(
        $label => 'inline imagemap',
        $length
        );
